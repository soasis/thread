...
project: str = 'ztd.thread'
copyright: str = "ThePhD & Shepherd's Oasis, LLC"
...
cpp_index_common_prefix: typing.List[str] = [
    'ztd::thread::__impl::', 'ztd::__impl::', 'ztd::__thread_detail::', 'ztd::'
]

# Breathe Configuration
#
breathe_projects: typing.Dict[str, object] = {}
breathe_default_project: str = "ztd.thread"
...
def run_cmake_doxygen():
	...
	print("[ztd.thread/documentation/conf.py] CMake Directory: %s" % cmake_dir)
	print("[ztd.thread/documentation/conf.py] XML Directory: %s" % xml_dir)

	try:
		retcode = subprocess.call(
		    "cmake -DZTD_THREAD_READTHEDOCS:BOOL=TRUE -DZTD_THREAD_DOCUMENTATION:BOOL=TRUE -DZTD_THREAD_DOCUMENTATION_NO_SPHINX:BOOL=TRUE ../../../..",
		    shell=True,
		    cwd=cmake_dir)
	except OSError as e:
	...
	try:
		retcode = subprocess.call("cmake --build . --target ztd.thread.documentation.doxygen",
		                          shell=True,
		                          cwd=cmake_dir)
	except OSError as e:
		sys.stderr.write("cmake generation execution failed: %s\n" % e)
	...
	breathe_projects["ztd.thread"] = xml_dir
...
def generate_doxygen_xml(app):
	...
	if read_the_docs_build:
		print(
		    "[ztd.thread/documentation/conf.py] Detected READTHEDOCS environment variable: running cmake from conf.py"
		)
		run_cmake_doxygen()
